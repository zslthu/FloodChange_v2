...
for window=1:2:max_window
    ...
    F_rain((window+1)/2)=(1/(T-window-1))*nansum((flutt_rain((window+1)/2,window+1-0.5*(window+1):T-0.5*(window+1))).^2); %Squared rainfall fluctuations (Eq.3)
    flutt_flow((window+1)/2,:)=flow_int-flow_mean((window+1)/2,:);
    F_flow((window+1)/2)=(1/(T-window-1))*nansum((flutt_flow((window+1)/2,window+1-0.5*(window+1):T-0.5*(window+1))).^2); %Squared streamflow fluctuations (Eq.4)
    F_rain_flow((window+1)/2)=(1/(T-window-1))*nansum(flutt_rain((window+1)/2,window+1-0.5*(window+1):T-0.5*(window+1)).*flutt_flow((window+1)/2,window+1-0.5*(window+1):T-0.5*(window+1))); %Bivariate rainfall-streamflow fluctuations (Eq.7)
